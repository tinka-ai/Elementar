      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <Link href="/" className={`${fxLink} text-sky-400`}>
              {t("header.nav.home")}
            </Link>
            <Link href="/domenii" className={fxLink}>
              {t("header.nav.domains")}
            </Link>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-t border-white/10">
      <div className="flex items-center justify-around py-3 px-4 max-w-md mx-auto">
        ...
        <Link
          href="tel:+37379010277"
          className={`flex flex-col items-center gap-1 text-xs text-gray-300 hover:text-white ${fxIcon}`}
          aria-label={t("aria.callNow")}
        >
          <div className="p-2 rounded-lg bg-white/10">
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
        ...
        <Link
          href={`mailto:office@elementar.md?subject=${encodeURIComponent(t("email.subject"))}&body=${encodeURIComponent(t("email.body"))}`}
          className={`flex flex-col items-center gap-1 text-xs text-gray-300 hover:text-white ${fxIcon}`}
